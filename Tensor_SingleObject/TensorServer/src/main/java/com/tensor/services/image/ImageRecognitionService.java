...
public class ImageRecognitionService {
	...
	public String parseImage(MultipartFile multipartFile) throws InterruptedException, ExecutionException {
		CompletableFuture<String> output= CompletableFuture.supplyAsync(() ->{
			List<String> labels = new ArrayList<String>();
			String modelDir = "E:\\Ankit\\TensorFlow";
			String result = "";
			try {
				byte[] graphDef = new byte[4096];
				graphDef = readAllBytesOrExit(Paths.get(modelDir, "tensorflow_inception_graph" + ".pb"));
				labels = readAllLinesOrExit(Paths.get(modelDir, "imageData" + ".txt"));
				//byte[] imageBytes = readAllBytesOrExit(Paths.get(imageTO.getFilePath(), imageTO.getFileName()));
				byte[] imageBytes = multipartFile.getBytes();
				try (Tensor<Float> image = constructAndExecuteGraphToNormalizeImage(imageBytes)) {
					float[] labelProbabilities = executeInceptionGraph(graphDef, image);
					int bestLabelIdx = maxIndex(labelProbabilities);
					result = String.format("Object: %s, Probability: %.2f%%", labels.get(bestLabelIdx),
							labelProbabilities[bestLabelIdx] * 100f);
					
					//  System.out.println( String.format("Object:%s, Probability:%.2f%%", labels.get(bestLabelIdx), labelProbabilities[bestLabelIdx] * 100f));
	...
	/*
	 * public boolean createImageData(ImageEntity entity) { try{
	 * entityManager.persist(entity); } catch (Exception e) { } return true; }
	 * 
	 * public ImageDetectionTO getImageData(String token) throws VGSException{
	 * ImageDetectionTO imageDetectionTO=new ImageDetectionTO(); try{
	 * List<ImageEntity> imageEntityList =entityManager.
	 * createQuery("select image from ImageEntity image where image.token=:id").
	 * setParameter("id", token).getResultList(); if(imageEntityList!=null &&
	 * !imageEntityList.isEmpty()){
	 ...
	 * imageDetectionTO.setToken(imageEntityList.get(0).getToken()); } } catch
	 * (Exception e) { throw new VGSException("Error fetching image data", e); }
